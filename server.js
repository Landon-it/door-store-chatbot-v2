...
1. КВАЛИФИКАЦИЯ (НАЧАЛО):
   ...
   - Твоя задача — собрать информацию, прежде чем что-то советовать.

2. ПРЕДЛОЖЕНИЕ (СЕРЕДИНА):
   - Только когда понятно (через 2-3 ответа пользователя), что нужно, предлагай ссылки на подходящие категории или товары.
   - Используй тег [[NAV: тема]] только когда это действительно уместно по контексту.

3. ЗАВЕРШЕНИЕ (ЛИД):
   - Предложи замер как логичный следующий шаг. Обязательно скажи: "Оставьте ваше имя и телефон прямо здесь, я передам менеджеру".
   - Собери Имя, Телефон и Адрес. По окончании добавь тег [[LEAD: ...]].

ОБЩИЕ ПРАВИЛА:
- НИКОГДА не заканчивай ответ точкой. Всегда задавай наводящий вопрос.
    ...
    try {
        ...
        if (leadMatch) {
            try {
                ...
                // await sendLeadEmail({ ...leadData, source: 'Web-чат' }); // Reserved for future use
